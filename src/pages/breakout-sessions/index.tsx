...
export default function BreakoutSessions() {
  ...
  const renderCard = (card: CardDetails, index: number) => (
    ...
    >
      ...
      <p className="absolute bottom-2 left-2 text-white bg-black px-2 py-1 rounded">{card.title}</p>
  ...
  return (
    <section>
      ...
      <div className="w-full">
        <div className="grid grid-cols-2 gap-5">{cardDetails.slice(0, 6).map(renderCard)}</div>
      </div>
      ...
      <div className="w-full mb-10">
        <div className="grid grid-cols-2 gap-5">{cardDetails.slice(6).map(renderCard)}</div>
      </div>

      {/* Register section */}
      <div className="bg-green bg-opacity-20 w-full mt-10">
